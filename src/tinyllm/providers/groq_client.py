"""Async Groq API client for TinyLLM.

Provides async interface to Groq's high-speed inference API with connection pooling,
retry logic, rate limiting, and support for all Groq features including:
...
logger = get_logger(__name__, component="groq_client")
metrics = get_metrics_collector()


# Global connection pool for Groq client reuse
_client_pool: dict[str, "GroqClient"] = {}
_pool_lock = asyncio.Lock()


async def get_shared_groq_client(
    api_key: Optional[str] = None,
    timeout_ms: int = 30000,
    max_retries: int = 3,
    rate_limit_rps: float = 30.0,
) -> "GroqClient":
    """Get or create a shared GroqClient for the given configuration.
    ...
    Raises:
        ValueError: If no API key provided and GROQ_API_KEY env var not set.
    """
    async with _pool_lock:
        cache_key = f"{api_key or 'env'}"
        if cache_key not in _client_pool:
            logger.info(
                "creating_shared_groq_client",
                timeout_ms=timeout_ms,
                rate_limit_rps=rate_limit_rps,
            )
            _client_pool[cache_key] = GroqClient(
                api_key=api_key,
                timeout_ms=timeout_ms,
...
async def close_all_groq_clients() -> None:
    """Close all pooled clients. Call during application shutdown."""
    logger.info("closing_all_groq_clients", client_count=len(_client_pool))
    async with _pool_lock:
        ...
        try:
            ...
            async with self._lock:
                ...
                if self.failures >= self.failure_threshold:
                    ...
                    self.state = "open"
                    metrics.increment_circuit_breaker_failures(model="groq")
            raise
...
class ChatCompletionRequest(BaseModel):
    """Request for Groq chat completions endpoint."""
...
class Usage(BaseModel):
    ...
    total_tokens: int
    prompt_time: Optional[float] = None  # Groq-specific: time in seconds
    completion_time: Optional[float] = None  # Groq-specific: time in seconds
    total_time: Optional[float] = None  # Groq-specific: time in seconds
...
class ChatCompletionResponse(BaseModel):
    ...
    x_groq: Optional[dict[str, Any]] = None  # Groq-specific metadata
...
class GroqClient:
    """Async client for Groq API with connection pooling, rate limiting, and circuit breaker.

    Groq provides high-speed inference for LLMs with an OpenAI-compatible API.
    This client supports chat completions, streaming, tool calling, and vision.
    """
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "Groq API key must be provided via api_key parameter or GROQ_API_KEY environment variable"
            )
                ...
                for attempt in range(self.max_retries + 1):
                    try:
                        ...
                        if result.usage:
                            ...
                            # Track Groq-specific timing metrics
                            if result.usage.prompt_time:
                                self._total_prompt_time += result.usage.prompt_time
                            if result.usage.completion_time:
                                self._total_completion_time += result.usage.completion_time
                            ...
                            # Log high-speed inference metrics
                            if result.usage.completion_time and result.usage.completion_tokens:
                                tokens_per_second = (
                                    result.usage.completion_tokens / result.usage.completion_time
                                )
                                logger.info(
                                    "groq_inference_speed",
                                    model=model,
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        Returns:
            ...
            and Groq-specific metrics like average tokens per second.
        """
        avg_tokens_per_second = 0.0
        if self._total_completion_time > 0:
            avg_tokens_per_second = self._total_tokens / self._total_completion_time
        ...
        return {
            ...
            "total_prompt_time": self._total_prompt_time,
