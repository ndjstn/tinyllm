"""Async Mistral AI client for TinyLLM.

Provides async interface to Mistral AI API with connection pooling,
retry logic, rate limiting, and support for all Mistral features including:
...
logger = get_logger(__name__, component="mistral_client")
metrics = get_metrics_collector()


# Global connection pool for Mistral client reuse
_client_pool: dict[str, "MistralClient"] = {}
_pool_lock = asyncio.Lock()


async def get_shared_mistral_client(
    api_key: Optional[str] = None,
    timeout_ms: int = 60000,
    max_retries: int = 3,
    rate_limit_rps: float = 5.0,
) -> "MistralClient":
    """Get or create a shared MistralClient.
    ...
    Returns:
        Shared MistralClient instance.
    """
    key = api_key or os.environ.get("MISTRAL_API_KEY", "default")
    async with _pool_lock:
        if key not in _client_pool:
            logger.info(
                "creating_shared_mistral_client",
                timeout_ms=timeout_ms,
                rate_limit_rps=rate_limit_rps,
            )
            _client_pool[key] = MistralClient(
                api_key=api_key,
                timeout_ms=timeout_ms,
...
async def close_all_mistral_clients() -> None:
    """Close all pooled clients. Call during application shutdown."""
    logger.info("closing_all_mistral_clients", client_count=len(_client_pool))
    async with _pool_lock:
        ...
        try:
            ...
            async with self._lock:
                ...
                if self.failures >= self.failure_threshold:
                    ...
                    self.state = "open"
                    metrics.increment_circuit_breaker_failures(model="mistral")
            raise
...
class ImageContent(BaseModel):
    ...
    image_url: Union[ImageUrl, str]  # Mistral accepts both formats
...
class SafeMode(str, Enum):
    """Safety mode for content filtering."""
...
class ChatCompletionRequest(BaseModel):
    """Request for Mistral chat completions endpoint."""
    ...
    safe_mode: SafeMode = SafeMode.NONE
...
class ChatCompletionResponse(BaseModel):
    """Response from Mistral chat completions endpoint."""
...
class EmbeddingRequest(BaseModel):
    """Request for Mistral embeddings endpoint."""
...
class EmbeddingResponse(BaseModel):
    """Response from Mistral embeddings endpoint."""
...
class MistralClient:
    """Async client for Mistral AI API with advanced features."""
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "Mistral API key must be provided or set in MISTRAL_API_KEY env var"
            )
    ...
    async def chat_completion(
        ...
        Args:
            ...
            safe_prompt: Whether to use safe prompt mode.
            safe_mode: Safety mode for content filtering.
    ...
    async def list_models(self) -> list[dict[str, Any]]:
        ...
        """
        client = await self._get_client()
        response = await client.get(f"/{self.API_VERSION}/models")
        response.raise_for_status()
        data = response.json()
    ...
    def create_image_message(
        ...
        """Create a message with text and images for Pixtral models.
